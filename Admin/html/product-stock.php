                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <h6 class="card-subtitle mb-3">In here the user (you), can view, and edit the quantity of the products listed over the product catalog. This will serve as the inventory.</h6>
                            </div>
                                <?php

                                // Fetch inventory data joined with product information
                                $sql_inventory = "SELECT i.inventoryid, i.productid, i.quantity, p.Name AS productname, p.product_image AS productimage
                                                    FROM tbl_inventory AS i
                                                    JOIN tbl_product AS p ON i.productid = p.ProductID";
                                $stmt_inventory = $pdo->query($sql_inventory);
                                $inventoryItems = $stmt_inventory->fetchAll(PDO::FETCH_ASSOC);
                                ?>
                                    ...
                                    <table id="default_order" class="table border table-striped table-bordered text-nowrap"
                                        ...
                                        <tbody>
                                            <?php foreach ($inventoryItems as $inventoryItem): ?>
                                                <tr>
                                                    <td><?php echo $inventoryItem['inventoryid']; ?></td>
                                                    <td>
                                                        <img src="../html/Include/<?php echo $inventoryItem['productimage']; ?>" class="img-fluid rounded-4" alt="Product Image" style="max-width: 100px; max-height: 100px; display: block; margin-top: 5px;">
                                                    </td>
                                                    <td><?php echo $inventoryItem['productname']; ?></td>
                                                    <td><?php echo $inventoryItem['quantity']; ?></td>
                                                    <td>
                                                        <a href="product-stock-edit.php?inventory_id=<?php echo $inventoryItem['inventoryid']; ?>" class="btn waves-effect waves-light btn-rounded btn-primary">Edit Quantity</a>
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
                                        </tbody>
